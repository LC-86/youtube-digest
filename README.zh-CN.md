...
若选择目录时 Chrome 提示“含有系统文件”，请改选普通子文件夹。Chrome 不允许授权整个下载、桌面、文稿或用户主目录。也可点击「另存为单个文件…」，无需先授权目录，由系统窗口选择位置并确认同名覆盖；不会更改默认目录。系统保护目录仍受 Chrome 限制。

获取字幕后，点击字幕栏的「保存 Markdown」。独立保存窗口会固定当前视频内容，供你选择模板、修改属性、检查 Markdown 预览、选择本地目录，再点击「下载 Markdown」。可以记住默认目录，浏览器权限失效时需要再次授权。文件名使用修改后的 `title` 加 `.md`；系统不允许的字符会替换为下划线并显示实际文件名。同名文件必须确认后才覆盖，取消不会改动原文件。此功能直接写入所选目录，不经过 Chrome 下载列表，也不要求安装本地伴侣。

正文以实际获取的字幕为准，不采用侧栏中文或双语翻译，不添加时间戳。保留字幕内已有姓名及说话人切换标记。旧缓存可能已丢失切换标记，保存窗口会提示该限制。

需要补充姓名时，点击「用模型识别姓名」。它使用设置中选择的 AI 服务，将视频信息和字幕分批提交，返回候选姓名和对应原文。请检查并逐项确认，或者移除不采用的候选；只有确认后的标注才会进入文件，字幕文字不由模型改写。模型无法识别时可手动标注或保留原文，失败不会丢失属性编辑。姓名判断基于文字，不是音频声纹识别。

模板支持新建、复制、删除、属性增删排序和正文编辑，也可导入／导出 Web Clipper JSON 模板。支持的变量为 `title`、`url`、`author`、`site`、`published`、`date`、`time`、`description`、`language`、`domain`、`content`，以及 `date:"YYYY-MM-DD"` 日期过滤器。`content` 对应逐字稿，`title` 文本属性必须保留。属性类型包括文本、列表、数字、复选框、日期和日期时间；列表每行一项或使用 JSON 数组。发布日期获取不到时留空，可手动补充。

导入不是完整兼容 Web Clipper：不执行网页选择器、Prompt 变量、条件循环或其他过滤器，需先修正这些规则；原路径、文件名规则、自动匹配及追加／日记行为会显示转换说明，需要确认后才能保存模板。可从设置页的「管理 Markdown 保存模板」单独进入。保存窗口内的属性和姓名标注会在当前窗口刷新后恢复；关闭窗口后请从字幕栏重新发起保存。
...
1. 点击「使用 ChatGPT 登录」。本地伴侣会在浏览器中打开 ChatGPT 授权页面，你用自己的账号登录并批准授权。
2. 等待期间设置页显示「正在授权」。页面会自动更新，可以保持打开。
3. 批准后，设置页显示「已连接」，并给出打码的账号标识。

授权使用与 OpenAI Codex CLI 相同的浏览器登录方式，回跳地址只指向这台 Mac 上的本地回环端口。整个流程由本地伴侣负责：扩展不会收到授权链接、授权码或任何令牌，也不保存任何账号凭据。登录凭据（刷新令牌）只保存在这台 Mac 的钥匙串中。

恢复操作都是显式的：

- 「取消授权」可以中止未完成的登录。
- 放弃的授权会在十分钟后过期；设置页会说明原因并允许再次登录。被拒绝的授权也会同样显示。
- 「需要重新连接」表示已保存的登录已失效（例如修改了密码）。可以重新登录，或断开连接。
- 「断开连接」会从钥匙串中删除已保存的凭据，卡片回到未登录状态。在 Chrome 中重置扩展数据不会删除该凭据，因为钥匙串条目属于本地伴侣而不是扩展；请使用断开连接。

### 选择 Codex 模型

登录后，打开设置页中的「AI 服务」卡片，选择 **ChatGPT / Codex**：

1. 点击「获取模型」。已安装的本地伴侣会按规范名称返回它支持的 Codex 模型列表。
2. 选择其中一个模型并点击「保存设置」。该选择只保存在当前 Chrome 个人资料中，重新打开设置页会恢复所选的服务和模型，你的 DeepSeek 密钥也保持原值不受影响。
3. DeepSeek 与 ChatGPT / Codex 之间的切换只是一次选择，另一方的设置不会被删除，随时可以切换回来而无需重新登录。

列表本身不代表的两件事：

- 列表来自本地伴侣而不是你的账号，列出的模型不保证包含在你的 ChatGPT 套餐中。最终以第一次真实请求为准；如果请求提示模型不可用，请重新获取模型、另选一个，或重新登录。
- 如果已安装的本地伴侣不再提供你保存的模型，设置页打开时会说明，并引导你重新获取模型。
...
- 两种 AI 服务：使用你自己的 API Key 的 DeepSeek V4 Flash，或通过本地伴侣使用的 ChatGPT / Codex 订阅。设置中所选的服务会服务所有 AI 功能。
- 可以安装可选的 macOS ChatGPT / Codex 本地伴侣；设置页会显示它是否已就绪、不可用或不兼容。已就绪后，可以使用 ChatGPT 账号登录、取消授权、重新连接或断开连接，也可以加载伴侣的 Codex 模型列表并选择保存一个模型。保存后，所有 AI 功能都会通过所选模型运行。
