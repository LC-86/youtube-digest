...
本次更新加入了账号登录和模型选择：设置页可以确认本地伴侣已安装、版本兼容并且可以通信；显示已就绪后，即可使用你的 ChatGPT 账号登录，并从本地伴侣提供的 Codex 模型中选择一个保存。通过所选模型发起 AI 请求会在后续更新中提供；在此之前，AI 功能继续使用 DeepSeek。
...
通过该连接发起 AI 请求会在后续更新中提供；在此之前，AI 功能继续使用 DeepSeek。
...
- 可以安装可选的 macOS ChatGPT / Codex 本地伴侣；设置页会显示它是否已就绪、不可用或不兼容。已就绪后，可以使用 ChatGPT 账号登录、取消授权、重新连接或断开连接，也可以加载伴侣的 Codex 模型列表并选择保存一个模型。通过它发起 AI 请求会在后续更新中提供。
