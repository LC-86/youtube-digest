...
AI requests through the companion arrive in a following update and will be documented here before they carry any transcript content. Removing the sign-in credential is explicit: use Disconnect in Settings. Resetting extension data in Chrome does not remove the Keychain credential, because it belongs to the companion, not the extension.
